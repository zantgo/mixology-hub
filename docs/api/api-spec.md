...
- Cursor is opaque to clients - they should treat it as an arbitrary string
 ...
 5. **Idempotency Keys for State-Mutating Operations:** All POST, PUT, PATCH, and DELETE endpoints that modify state support the `Idempotency-Key` header to prevent duplicate operations from network retries or double-clicks. Idempotency keys follow the unified format defined in ADR 0012: `idempotency:v2:{userId}:{operation}:{uuid}` (e.g., `idempotency:v2:user_<ID>:cocktail:prepare:<UUID>` and `idempotency:v2:user_<ID>:preparation:undo:<UUID>`) to prevent cross-contamination between different users and operations while ensuring consistency between online clicks and offline sync batch processing.
    ...
    **Behavior:**
    ...
    - Required for preparation, undo, inventory updates, and batch sync operations
...
**Senior Architectural Decision: Bulk Sync Rate Limit Exemption**
**Explicit Trade-off:** To process offline queues seamlessly, the `/offline/sync` endpoint is explicitly exempted from the standard per-action rate limiters (e.g., preparation throttles). Instead, it relies on a specialized Bulk Rate Limiter (e.g., max 2 sync payloads per minute) and strict payload bounds (max 100 operations). We trade granular per-action throttling for offline UX recovery stability.

**Headers:**
- `Idempotency-Key`: Optional UUID string for the entire batch operation (batch-level idempotency)

**Senior Architectural Decision: Two-Level Idempotency for Batch Sync**
**Explicit Trade-off:** Batch sync operations require dual-layer idempotency protection: 1) Batch-level to prevent duplicate submission of the entire payload, and 2) Operation-level to allow partial retries of failed individual operations. The Global Interceptor (ADR 0012) handles batch-level idempotency via the `Idempotency-Key` header, while the sync service internally checks operation-level idempotency using `sync:op:{client_operation_id}` keys. This allows retrying failed operations within a batch while preventing duplicate processing of successful ones.
...
"quantity": "500",
      "unit": "ml", // REQUIRED: The UnitConverterService needs this for mathematical conversions
