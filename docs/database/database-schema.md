...
USERS ||--o{ REFRESH_TOKENS : "has_sessions"
 ...
 USERS {
  ...
  boolean enable_offline_mode DEFAULT false "enable offline sync features"
  }
...
INGREDIENT_RELATIONS {
 ...
 boolean is_deleted DEFAULT false "soft delete flag"
  ...
  uuid ingredient_id FK
    ...
    decimal(10,4) quantity "CHECK (quantity >= 0) /* Prevents offline sync from causing negative inventory */"
...
SYNC_OPERATIONS {
  ...
  timestamp created_at
  timestamp updated_at
  -- Note: Idempotency checking now handled by unified_idempotency table (ADR 0012)
  -- This table remains as a data queue for offline sync operations
}
