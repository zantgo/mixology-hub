...
- **Boundary Enforcement Risk**: Precision drift (ADR 0015) combined with GDPR rating subtraction may violate 0-5 bounds
  ...
  - **Senior Architectural Decision: Asynchronous Rating Corruption during GDPR Deletion**
  - **Explicit Trade-off:** We explicitly accept that the asynchronous rating-subtraction logic is fundamentally flawed due to the immediate destruction of pivot data via PostgreSQL `ON DELETE CASCADE`. We mandate that we will NOT block the GDPR `DELETE` transaction to calculate thousands of averages synchronously, as it violates prompt deletion SLAs and risks database timeouts. We trade real-time rating accuracy for guaranteed, fast GDPR compliance, relying ENTIRELY on the Nightly Rating Recalibration Cron Job (ADR 0015) to silently heal the database averages overnight.
