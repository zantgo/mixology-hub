import { Controller, Post, Get, Req, Res, UseGuards, HttpCode, HttpStatus } from '@nestjs/common';
...
export class McpController {
  ...
  async sse(@Req() req: Request, @Res() res: Response) {
    ...
    res.write(`data: ${JSON.stringify({ type: 'connected', sessionId: session.ticketId })}\n\n`);
      ...
      try {
        ...
        switch (msg.method) {
          ...
          case 'tools/call':
            ...
            res.write(`data: ${JSON.stringify({ type: 'tool_result', id: msg.id, result })}\n\n`);
            break;

          default:
            res.write(`data: ${JSON.stringify({ type: 'error', message: `Unknown method: ${msg.method}` })}\n\n`);
        }
      } catch (err: any) {
