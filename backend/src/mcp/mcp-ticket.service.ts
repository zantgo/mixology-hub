...
export class McpTicketService {
  ...
  async validateTicket(ticket: string): Promise<McpSession> {
    ...
    const session: McpSession = {
      userId,
      ticketId: crypto.createHash('sha256').update(ticket).digest('hex').substring(0, 16),
      createdAt: new Date(),
    };

    const sessionKey = `mcp:session:${session.ticketId}`;
    await this.cacheManager.set(sessionKey, JSON.stringify(session), this.SESSION_TTL_MS);
