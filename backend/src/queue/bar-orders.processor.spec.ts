...
function makeIngredient(overrides: Partial<Ingredient> = {}): Ingredient {
  return { id: 'ing-1', name: 'Vodka', baseUnit: 'ml', density: new Decimal(1.0), allowMassVolumeConversion: true, ...overrides } as Ingredient;
}

function makeCocktailIngredient(overrides: any = {}): CocktailIngredient {
  return { amount: 50, unit: 'ml', ingredient: makeIngredient(), ...overrides } as CocktailIngredient;
}

function makeCocktail(overrides: any = {}): Cocktail {
  return { id: 'cocktail-1', name: 'Test Cocktail', ingredients: [makeCocktailIngredient()], ...overrides } as Cocktail;
}

function makeLog(overrides: any = {}): PreparationLog {
  return { id: 'log-1', status: 'queued', undone: false, ...overrides } as PreparationLog;
}

function makeJob(overrides: any = {}): Job {
  return { id: 'job-1', data: { cocktailId: 'cocktail-1', bartenderId: 'user-1', preparationLogId: 'log-1', servings: 1, force: false, ...overrides } } as Job;
}
...
function createMocks() {
  ...
  const tm = { findOne: jest.fn(), save: jest.fn(), createQueryBuilder: jest.fn().mockReturnValue(qb) };
  const ds = { transaction: jest.fn().mockImplementation(async (cb: Function) => { try { return await cb(tm); } catch (e) { throw e; } }) };
  const prepRepo = { update: jest.fn().mockResolvedValue({ affected: 1 }) };
  const processor = new BarOrdersProcessor(ds as any, {} as any, {} as any, prepRepo as any, new UnitConverterService());
...
describe('BarOrdersProcessor - prepare', () => {
  it('should successfully prepare a cocktail with sufficient stock', async () => {
    ...
    const lastSave = (tm.save as jest.Mock).mock.calls.at(-1)?.[0];
  ...
  it('should fail with insufficient stock', async () => {
    const { qb, tm, processor } = createMocks();
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(makeCocktail({ ingredients: [makeCocktailIngredient({ amount: 500, unit: 'ml' })] }));
    qb.getOne.mockResolvedValue({ quantity: new Decimal(10) });
  ...
  it('should skip ingredients in force mode', async () => {
    ...
    const cocktail = makeCocktail({
      ingredients: [makeCocktailIngredient({ amount: 500, unit: 'ml', ingredient: ing1 }), makeCocktailIngredient({ amount: 30, unit: 'ml', ingredient: ing2 })],
    });
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(cocktail);
    qb.getOne.mockResolvedValueOnce({ quantity: new Decimal(10) }).mockResolvedValueOnce({ quantity: new Decimal(100) });
    qb.execute.mockResolvedValue({ affected: 1 });
  ...
  it('should throw when log not found', async () => {
    ...
    await expect(processor.process(makeJob())).rejects.toThrow('PreparationLog log-1 not found');
  });
  ...
  it('should throw when cocktail not found', async () => {
    ...
    await expect(processor.process(makeJob())).rejects.toThrow('Cocktail cocktail-1 not found');
    expect(prepRepo.update).toHaveBeenCalledWith('log-1', { status: 'failed_other' });
  });

  it('should throw when recipe is corrupt', async () => {
    const { tm, prepRepo, processor } = createMocks();
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(makeCocktail({ ingredients: [{ amount: 50, unit: 'ml', ingredient: null }] }));
    await expect(processor.process(makeJob())).rejects.toThrow('Cocktail recipe is corrupt');
    expect(prepRepo.update).toHaveBeenCalledWith('log-1', { status: 'failed_other' });
  });

  it('should fail when update returns 0 rows', async () => {
    const { qb, tm, processor } = createMocks();
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(makeCocktail());
    qb.getOne.mockResolvedValue({ quantity: new Decimal(100) });
  ...
  it('should convert units during preparation', async () => {
    ...
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(makeCocktail({ ingredients: [makeCocktailIngredient({ amount: 2, unit: 'oz', ingredient: ing })] }));
    qb.getOne.mockResolvedValue({ quantity: new Decimal(100) });
    qb.execute.mockResolvedValue({ affected: 1 });
  ...
  it('should multiply by servings', async () => {
    const { qb, tm, processor } = createMocks();
    const ing = makeIngredient({ baseUnit: 'ml' });
    tm.findOne.mockResolvedValueOnce(makeLog()).mockResolvedValueOnce(makeCocktail({ ingredients: [makeCocktailIngredient({ amount: 50, unit: 'ml', ingredient: ing })] }));
    qb.getOne.mockResolvedValue({ quantity: new Decimal(200) });
    qb.execute.mockResolvedValue({ affected: 1 });
...
describe('BarOrdersProcessor - undo', () => {
  it('should successfully undo a completed preparation', async () => {
    const { qb, tm, processor } = createMocks();
    const log = makeLog({ status: 'completed', deductedIngredients: [
      { ingredientId: 'ing-1', ingredientName: 'Vodka', amount: '50', unit: 'ml' },
      { ingredientId: 'ing-2', ingredientName: 'Lime Juice', amount: '30', unit: 'ml' },
    ]});
  ...
  it('should skip ingredients that were skipped in force prepare', async () => {
    const { qb, tm, processor } = createMocks();
    const log = makeLog({ status: 'completed', deductedIngredients: [
      { ingredientId: 'ing-1', ingredientName: 'Vodka', amount: '0', skipped: true },
      { ingredientId: 'ing-2', ingredientName: 'Lime Juice', amount: '30', unit: 'ml' },
    ]});
    tm.findOne.mockResolvedValue(log);
    qb.getOne.mockResolvedValue({ quantity: new Decimal(100) });
  ...
  it('should gracefully skip deleted inventory rows', async () => {
    const { qb, tm, processor } = createMocks();
    const log = makeLog({ status: 'completed', deductedIngredients: [
      { ingredientId: 'ing-1', ingredientName: 'Deleted', amount: '50', unit: 'ml' },
    ]});
    tm.findOne.mockResolvedValue(log);
  ...
  it('should throw when log not found', async () => {
    const { tm, processor } = createMocks();
    tm.findOne.mockResolvedValue(null);
    await expect(processor.process(makeJob({ type: 'undo' }))).rejects.toThrow('PreparationLog log-1 not found');
  });

  it('should throw when log is not completed', async () => {
    const { tm, processor } = createMocks();
    tm.findOne.mockResolvedValue(makeLog({ status: 'queued' }));
    await expect(processor.process(makeJob({ type: 'undo' }))).rejects.toThrow('status is queued');
  });

  it('should throw when already undone', async () => {
    const { tm, processor } = createMocks();
    tm.findOne.mockResolvedValue(makeLog({ status: 'completed', undone: true }));
    await expect(processor.process(makeJob({ type: 'undo' }))).rejects.toThrow('already been undone');
  });

  it('should throw when no deductions', async () => {
    const { tm, processor } = createMocks();
    tm.findOne.mockResolvedValue(makeLog({ status: 'completed', deductedIngredients: [] }));
    await expect(processor.process(makeJob({ type: 'undo' }))).rejects.toThrow('No deductions found');
  });
});
