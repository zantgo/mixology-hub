...
export class EnhancedTheCocktailDbService {
  ...
  async searchByName(name: string, options?: { bypassCache?: boolean }) {
    ...
    if (!this.isCircuitClosed()) {
      ...
      throw new InternalServerErrorException('External API is temporarily unavailable');
    }

    // Check rate limit
    if (!this.checkRateLimit()) {
      this.logger.warn(`Rate limit exceeded for TheCocktailDB API`);
      throw new InternalServerErrorException('Rate limit exceeded for external API');
    }
    ...
    try {
      ...
      const { data } = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/search.php?s=${encodeURIComponent(sanitizedName)}`, {
      ...
      if (error instanceof AxiosError) {
        if (error.code === 'ECONNABORTED') {
          ...
          throw new InternalServerErrorException('External API rate limit exceeded');
  ...
  async searchByIngredient(ingredient: string) {
    ...
    try {
      ...
      const { data } = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/filter.php?i=${encodeURIComponent(sanitizedIngredient)}`, {
          timeout: 10000,
        }),
      );
  ...
  async getCocktailById(id: string) {
    ...
    try {
      ...
      const { data } = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/lookup.php?i=${encodeURIComponent(id)}`, {
          timeout: 10000,
        }),
      );
      ...
      await this.cacheManager.set(cacheKey, sanitizedData.drinks?.[0] || null, 86400000); // 24 hours
  ...
  private sanitizeResponse(data: any): any {
    ...
    // Sanitize each drink
    const sanitizedDrinks = data.drinks.map((drink: any) => {
