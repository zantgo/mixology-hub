...
describe('BarInventoryService', () => {
  ...
  beforeEach(() => {
    ...
    service = new BarInventoryService(repo as any, ingRepo as any, new UnitConverterService(), ds as any);
  });
  ...
  it('should add a new ingredient to inventory', async () => {
    ...
    await service.addToInventory({ ingredientId: 'ing-1', quantity: 100, unit: 'ml' });
    expect(repo.create).toHaveBeenCalledWith({ ingredient: ing, quantity: expect.any(Decimal) });
  ...
  it('should convert units when adding with different unit', async () => {
    ...
    await service.addToInventory({ ingredientId: 'ing-1', quantity: 2, unit: 'oz' });
  ...
  it('should throw NotFoundException when ingredient missing', async () => {
    ...
    await expect(service.addToInventory({ ingredientId: 'nonexistent', quantity: 100, unit: 'ml' })).rejects.toThrow(NotFoundException);
  });
  ...
  it('should throw NotFoundException when item not found', async () => {
    repo.findOne.mockResolvedValue(null);
    await expect(service.getInventoryItem('nonexistent')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should throw when updating nonexistent item', async () => {
    repo.findOne.mockResolvedValue(null);
    await expect(service.updateInventoryItem('nonexistent', { quantity: 100, unit: 'ml' })).rejects.toThrow(NotFoundException);
  });
  ...
  it('should throw when removing nonexistent item', async () => {
    repo.findOne.mockResolvedValue(null);
    await expect(service.removeFromInventory('nonexistent')).rejects.toThrow(NotFoundException);
  });
  ...
  it('should bulk add items in transaction', async () => {
    ...
    const result = await service.bulkAdd([{ ingredientId: 'ing-1', quantity: 100, unit: 'ml' }]);
  ...
  it('should throw on missing ingredient in bulk add', async () => {
    ...
    await expect(service.bulkAdd([{ ingredientId: 'nonexistent', quantity: 100, unit: 'ml' }])).rejects.toThrow(NotFoundException);
  });
  ...
  it('should increment existing in bulk add', async () => {
    ...
    ds.transaction.mockImplementation(async (cb: Function) => {
      ...
      const mgr = {
        findOne: jest.fn().mockImplementation(() => {
          callCount++;
          return callCount === 1 ? Promise.resolve(ing) : Promise.resolve(existing);
        }),
        save: jest.fn().mockResolvedValue(existing),
      };
      return cb(mgr);
    });
    const result = await service.bulkAdd([{ ingredientId: 'ing-1', quantity: 50, unit: 'ml' }]);
    expect(result).toHaveLength(1);
  });
