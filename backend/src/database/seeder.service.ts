...
export class SeederService implements OnModuleInit {
  ...
  private async seedIngredients(): Promise<void> {
    const count = await this.ingredientRepository.count({ where: { isGlobal: true } });
    if (count > 0) {
      this.logger.log(`Ingredients already seeded (${count} global ingredients found), skipping.`);
      return;
    }
    ...
    this.logger.log(`Seeded ${entities.length} global ingredients successfully.`);
  }
}