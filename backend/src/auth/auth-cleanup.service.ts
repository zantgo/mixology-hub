...
export class AuthCleanupService {
  ...
  async cleanupExpiredResetTokens(): Promise<void> {
    const result = await this.userRepository
      .createQueryBuilder()
      .update(User)
      .set({ resetPasswordToken: null as any, resetPasswordExpires: null as any })
      .where('resetPasswordExpires < :now', { now: new Date() })
      .andWhere('resetPasswordToken IS NOT NULL')
      .execute();

    if (result.affected) {
      this.logger.log(`Cleared ${result.affected} expired password reset tokens`);
    }
  }
  ...
  async cleanupStaleVerificationTokens(): Promise<void> {
    ...
    const result = await this.userRepository
      .createQueryBuilder()
      .update(User)
      .set({ emailVerificationToken: null as any })
    ...
    if (result.affected) {
      this.logger.log(`Cleared ${result.affected} stale email verification tokens`);
    }
  }
}
