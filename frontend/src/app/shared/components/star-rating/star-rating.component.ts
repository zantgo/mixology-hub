    ...
    <div class="star-rating" [class.interactive]="interactive" [attr.aria-label]="ariaLabel">
      @for (star of stars; track star) {
        <button
          ...
          [class.half]="!interactive && star === Math.ceil(value) && value % 1 >= 0.25 && value % 1 < 0.75"
...
export class StarRatingComponent {
  ...
  onRate(score: number): void {
    ...
    this.http.post<{ averageRating: number; userRating: number; ratingCount: number }>(
      `${environment.apiUrl}/cocktails/${this.cocktailId}/rate`,
      { score }
    ).subscribe({
      next: (res) => {
