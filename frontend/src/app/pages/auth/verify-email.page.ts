    ...
    <div class="auth-container">
      <div class="auth-card card">
        <div class="auth-header">
          <app-icon [name]="success() ? 'check-circle' : 'mail'" [size]="48" [color]="success() ? 'var(--color-success)' : 'var(--color-primary)'" />
...
export class VerifyEmailPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.auth.verifyEmail(token).subscribe({
      ...
      error: (err) => {
        this.loading.set(false);
        this.error.set(err.error?.message || 'Failed to verify email. The link may be invalid or expired.');
      },
    });
  }
}
