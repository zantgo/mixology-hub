...
@Component({
  ...
  template: `
    <article class="cocktail-card card" role="article" [attr.aria-labelledby]="titleId">
        ...
        <div class="card-image">
          ...
          <div class="card-badges">
            @if (cocktail.makeability) {
              <app-badge
                [type]="$any(cocktail.makeability)"
                [label]="makeabilityLabel"
                [icon]="makeabilityIcon"
              />
            }
            @if ($any(cocktail.source) === 'ai') {
...
export class CocktailCardComponent {
  ...
  readonly titleId = 'cocktail-title-' + Math.random().toString(36).slice(2, 9);
