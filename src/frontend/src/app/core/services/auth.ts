...
export class Auth {
  ...
  register(body: {
    ...
    password: string;
    displayName?: string;
  }): Observable<{ user: UserProfile } & AuthTokens> {
    return this.http
      .post<{ user: UserProfile } & AuthTokens>(`${this.apiUrl}/register`, body)
      .pipe(tap((tokens) => this.storeTokens(tokens)));
  }

  login(body: { email: string; password: string }): Observable<{ user: UserProfile } & AuthTokens> {
    return this.http
      .post<{ user: UserProfile } & AuthTokens>(`${this.apiUrl}/login`, body)
      .pipe(tap((tokens) => this.storeTokens(tokens)));
  }
