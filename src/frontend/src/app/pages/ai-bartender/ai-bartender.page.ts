      ...
      <div class="ai-form card">
        ...
        <div class="form-section">
          <label class="checkbox-label">
            <input
              type="checkbox"
              [(ngModel)]="useOnlyMyBar"
              [disabled]="inventoryStore.items().length === 0 || aiStore.generating()"
            />
...
export class AiBartenderPage implements OnInit {
  ...
  constructor() {
    effect(() => {
      if (this.aiStore.generating()) return;
      if (this.aiStore.pendingRecipe()) {
        this.announcer.announce('recipe generated', 'assertive');
      }
    });
  }
