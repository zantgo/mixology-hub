  ...
  describe('convert', () => {
    ...
    describe('mass-to-volume conversions (with ingredient)', () => {
      ...
      it('should throw when mass-to-volume without ingredient', () => {
        expect(() => service.convert(100, 'g', 'ml')).toThrow(
          BadRequestException,
        );
      });

      it('should throw when volume-to-mass without ingredient', () => {
        expect(() => service.convert(100, 'ml', 'g')).toThrow(
          BadRequestException,
        );
      });
      ...
      it('should throw when ingredient does not allow mass-volume conversion', () => {
        ...
        expect(() => service.convert(100, 'g', 'ml', ingredient)).toThrow(
          BadRequestException,
        );
      });
    });
    ...
    describe('unsupported unit handling', () => {
      it('should throw for unsupported from-unit', () => {
        expect(() => service.convert(100, 'gallon', 'ml')).toThrow(
          BadRequestException,
        );
      });

      it('should throw for unsupported to-unit', () => {
        expect(() => service.convert(100, 'ml', 'gallon')).toThrow(
          BadRequestException,
        );
      });

      it('should throw for completely unknown unit', () => {
        expect(() => service.convert(100, 'barrel', 'liter')).toThrow(
          BadRequestException,
        );
      });
    });
