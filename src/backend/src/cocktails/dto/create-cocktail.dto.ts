...
class CreateCocktailIngredientDto {
  @ApiProperty({
    ...
    description: 'Ingredient ID from catalog',
  })
  @IsString()
  @IsNotEmpty()
  ingredientId: string;
  ...
  @ApiProperty({
    example: 'oz',
    description: 'Unit used for calculation (ml, oz, grams)',
  })
  @IsString()
  @IsNotEmpty()
  @IsIn(ALLOWED_UNITS)
  ...
  @ApiProperty({
    example: '2 oz',
    description: 'Full string for display purposes (UI label)',
  })
  @IsString()
  @IsNotEmpty()
  measure: string;
...
export class CreateCocktailDto {
  ...
  @ApiProperty({
    example: '11000',
    description:
      'Original external cocktail ID when forking from TheCocktailDB (UC 2.22 lineage tracking)',
    required: false,
  })
  @IsOptional()
  @IsString()
