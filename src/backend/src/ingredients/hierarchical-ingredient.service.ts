...
export class HierarchicalIngredientService implements OnApplicationBootstrap {
  ...
  async findBestMatch(
    ...
    if (includeHierarchical) {
      const hierarchicalMatch =
        await this.findHierarchicalMatch(normalizedName);
  ...
  async findSubstitutions(
    ...
    const hierarchicalSubs = await this.findHierarchicalSubstitutions(
      ingredient,
      minConfidence,
    );
    ...
    if (substitutions.length < maxSubstitutions) {
      const synonymSubs = await this.findSynonymSubstitutions(
        ingredient,
        minConfidence,
      );
  ...
  async validateIngredientHierarchy(ingredientId: string): Promise<{
    ...
    try {
      ...
      const circular = this.detectCircularReference(
      ...
      if (hierarchy.ancestors.length > 5) {
        issues.push(
          `Hierarchy depth is ${hierarchy.ancestors.length} levels deep, which may be excessive`,
        );
        suggestions.push(
          'Consider flattening the hierarchy for better performance',
        );
      }
      ...
      if (!hierarchy.ingredient.synonyms && hierarchy.siblings.length > 0) {
        suggestions.push(
          'Consider adding synonyms for better matching with sibling ingredients',
        );
      }
  ...
  async findCommonAncestor(
    ...
    const hierarchies = await Promise.all(
      ingredientIds.map((id) =>
        this.getAncestors(id).then((ancestors) => ({ id, ancestors })),
      ),
    );
    ...
    for (let i = 1; i < allAncestors.length; i++) {
      commonAncestorIds = commonAncestorIds.filter((id) =>
        allAncestors[i].includes(id),
      );
    }
  ...
  async expandIngredientQuery(ingredientName: string): Promise<string[]> {
    ...
    if (match) {
      ...
      if (match.ingredient.synonyms) {
        const synonyms = match.ingredient.synonyms
          .split(',')
      ...
      const hierarchy = await this.buildIngredientHierarchy(
        match.ingredient.id,
      );
  ...
  private async findExactMatch(
    ...
    if (ingredient) {
      await this.cacheManager.set(
        cacheKey,
        this.toPlainIngredient(ingredient),
        this.CACHE_TTL * 1000,
      );
    }
  ...
  private async findSynonymSubstitutions(
    ...
    for (const synonym of synonyms) {
      const synonymIngredient = await this.findExactMatch(
        this.normalizeName(synonym),
      );
  ...
  private async getAncestors(ingredientId: string): Promise<Ingredient[]> {
    ...
    while (currentId) {
      if (visited.has(currentId)) {
        this.logger.warn(
          `Circular reference detected in ingredient hierarchy at ${currentId}. Breaking traversal.`,
        );
        break;
      }
      ...
      if (visited.has(parent.id)) {
        this.logger.warn(
          `Circular reference detected in ingredient hierarchy at ${parent.id}. Breaking traversal.`,
        );
        break;
      }
  ...
  private detectCircularReference(
    ...
    return (
      ancestorIds.includes(ingredient.id) ||
      descendantIds.includes(ingredient.id)
    );
  }
  ...
  private checkUnitConsistency(hierarchy: {
    ...
    if (unitCount > 1) {
      ...
      const exampleIngredients = Object.values(units)[0]
        .slice(0, 3)
        .map((ing) => ing.name)
        .join(', ');
  ...
  async detectCycle(ingredientId: string, newParentId: string): Promise<void> {
    ...
    while (currentId) {
      if (visited.has(currentId)) {
        throw new BadRequestException(
          `Setting parent for ingredient ${ingredientId} to ${newParentId} would create a circular reference`,
        );
      }
      visited.add(currentId);
      const ingredient = await this.getIngredientById(currentId);
